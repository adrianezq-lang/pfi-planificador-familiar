...
function App() {
 ...
 return <div className="app-shell"><header className="app-header"><div className="app-header__inner"><div className="app-logo" aria-hidden="true">🏡</div><div className="app-brand"><h1>Planificador Familiar Inteligente</h1><p>Menús, compra, despensa y presupuesto</p></div><span className="app-version">PFI</span></div></header><Suspense fallback={<main className="page page-loading"><span>Abriendo…</span></main>}>{pantalla==='inicio'&&<Home menu={menu} planMensual={planMensual} semanaActiva={semanaActiva} navegar={cambiarPantalla}/>} {pantalla==='menu'&&<Menu menu={menu} planMensual={planMensual} semanaActiva={semanaActiva} seleccionarSemana={seleccionarSemana} mesActivo={mesActivo} cambiarMes={cambiarMes} excluirSemana={excluirSemana} generarNuevoMes={generarNuevoMes} reiniciarMes={reiniciarMes}/>} {pantalla==='compra'&&<Compra menu={menu} menuMes={menuMes} mesActivo={mesActivo} semanaActiva={semanaActiva}/>} {pantalla==='despensa'&&<Despensa/>} {pantalla==='recetas'&&<Recetas/>} {pantalla==='postres'&&<Postres/>} {pantalla==='catalogo'&&<CatalogoMercadona/>} {pantalla==='perfil'&&<Perfil/>}</Suspense><NavegacionRecetario pantalla={pantalla} cambiarPantalla={cambiarPantalla}/><BottomNav pantallaActual={pantalla === 'postres' ? 'recetas' : pantalla} cambiarPantalla={cambiarPantalla}/></div>;
}
export default App;