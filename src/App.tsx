...
export type Pantalla = 'inicio'|'menu'|'compra'|'despensa'|'recetas'|'catalogo'|'perfil';
function App() {
 const [pantalla,setPantalla]=useState<Pantalla>('inicio');
 const {menu,guardar,planMensual,guardarPlan,semanaActiva,seleccionarSemana,mesActivo,cambiarMes,excluirSemana}=useMenu();
 ...
 return <div className="app-shell">
  ...
  <Suspense fallback={<main className="page page-loading"><span>Abriendo…</span></main>}>
   ...
   {pantalla==='menu'&&<Menu menu={menu} guardarMenu={guardar} planMensual={planMensual} guardarPlan={guardarPlan} semanaActiva={semanaActiva} seleccionarSemana={seleccionarSemana} mesActivo={mesActivo} cambiarMes={cambiarMes} excluirSemana={excluirSemana}/>} 
   {pantalla==='compra'&&<Compra menu={menu}/>} {pantalla==='despensa'&&<Despensa/>} {pantalla==='recetas'&&<Recetas/>} {pantalla==='catalogo'&&<CatalogoMercadona/>} {pantalla==='perfil'&&<Perfil/>}
  </Suspense><BottomNav pantallaActual={pantalla} cambiarPantalla={cambiarPantalla}/>
 </div>;
}
export default App;
