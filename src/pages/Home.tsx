...
function Home({
  ...
  const cargarResumen = useCallback(async () => {
    ...
    try {
      ...
      const menusParaCalcular =
        semanasHastaAhora.length > 0 ? semanasHastaAhora : [menu];
      const resultados = await Promise.all(
        menusParaCalcular.map((menuSemana) =>
          generarCompraMercadona(menuSemana),
        ),
      );
      setPresupuesto(
        calcularResumenPresupuestoMensual(resultados, semanaActiva),
      );
    } catch {
      setPresupuesto({
        ...RESUMEN_VACIO,
  ...
  const reposicion = useMemo(
    () =>
      despensa
        ...
        .sort(
          (a, b) => calcularReposicion(b) - calcularReposicion(a),
        ),
    [despensa],
  );
          ...
          {menuHoy ? (
            <>
              ...
              <div className="meal-stack">
                ...
                />
                <MealRow
                  etiqueta="POSTRE COMIDA"
                  nombre={`${iconoRecetaPostre(obtenerRecetaPostre(menuHoy, 'comida'))} ${formatearPostreMenu(menuHoy, 'comida')}`}
                />
                <MealRow
                  etiqueta="POSTRE CENA"
                  nombre={`${iconoRecetaPostre(obtenerRecetaPostre(menuHoy, 'cena'))} ${formatearPostreMenu(menuHoy, 'cena')}`}
                />
              </div>
            </>
          ) : (
...
function MealRow({ etiqueta, nombre }: { etiqueta: string; nombre: string }) {
  return (
