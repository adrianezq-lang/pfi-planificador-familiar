...
function Home({ menu, planMensual, semanaActiva, navegar }: HomeProps) {
  ...
  const cargarResumen = useCallback(async () => {
    ...
    try {
      const semanasHastaAhora = planMensual.slice(0, Math.max(1, semanaActiva + 1)).filter((semana) => !semana.excluida).map((semana) => semana.menu);
      const menusParaCalcular = semanasHastaAhora.length > 0 ? semanasHastaAhora : [menu];
      const resultados = await Promise.all(menusParaCalcular.map((menuSemana) => generarCompraMercadona(menuSemana)));
      setPresupuesto(calcularResumenPresupuestoMensual(resultados, semanaActiva));
    } catch { setPresupuesto({ ...RESUMEN_VACIO, mostrarPresupuestoMensual: semanaActiva === 0 }); }
