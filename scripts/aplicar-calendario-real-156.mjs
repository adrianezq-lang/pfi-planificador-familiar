...
// La integración debe ser idempotente y dejar siempre los imports reales disponibles.
const calendarioImport = "import { claveMesActual, desplazarMes } from '../services/calendarioMensual';";
const calendarioImportRegex = /import \{[^\n]*\} from '\.\.\/services\/calendarioMensual';/;
if (calendarioImportRegex.test(hook)) {
  hook = hook.replace(calendarioImportRegex, calendarioImport);
} else if (!hook.includes(calendarioImport)) {
  hook = hook.replace(
...
if (!hook.includes('function asegurarCincoSemanas(')) {
  hook = hook.replace(
...
if (!hook.includes("useState(cargarMesActivo)")) {
  hook = hook.replace(
...
if (!hook.includes('function cambiarMes(delta: number)')) {
  hook = hook.replace(
...
// El plan antiguo queda reemplazado por el plan por mes. Evitamos noUnusedLocals.
const inicioPlanAntiguo = hook.indexOf('function cargarPlan(): SemanaMenu[] {');
const finPlanAntiguo = hook.indexOf('function cargarIndiceSemana(): number {');
if (inicioPlanAntiguo >= 0 && finPlanAntiguo > inicioPlanAntiguo && hook.includes('function cargarPlanParaMes(')) {
  hook = hook.slice(0, inicioPlanAntiguo) + hook.slice(finPlanAntiguo);
}
